...
Штаб продуктов: цели, GitHub Project и оркестратор. Сейчас **active** — семья **win-predict-ai**; **telegram-bots** и **games** — stub (Goal на доске без child, пока не подключим репо в registry).
...
Goal Issues — в этом репо. Child issues — в таблице выше (только product win-predict-ai).
...
Колонки: **Inbox** → **In Progress** → **Review** → **Done**. Привязана к этому репо и шести рабочим win-predict.

## Статус

Таймер `board-watch` на VPS раз в 2 мин видит карточки **In Progress** и будит оркестратор (Goal) или воркера (child — в т.ч. ручной issue в рабочем репо без Goal). Stub-Goal → `needs_human` и Review. После PR local-ревьюер либо оставляет child в In Progress (правки), либо двигает в **Review**. Замечания пиши в issue и верни карточку в In Progress. **Review → In Progress** без комментария — проверка/подтягивание `main` в PR; конфликт → MODE B, иначе снова Review. Когда ок — напиши «релизь» (или «можно релизить» / «отправляем на релиз») и перенеси в **In Progress**: вотчер поднимет версию, обновит ченджлог, смержит PR и поставит **Done**.
