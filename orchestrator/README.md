...
4. Приёмка: человек **Review → Ready to Release**. Вотчер: bump `package.json` + CHANGELOG → подтянуть base в PR → `gh pr merge --squash` → **Done**. Конфликт с main → **In Progress** (воркер MODE B). Goal в Ready to Release релизит оставшиеся open child.
