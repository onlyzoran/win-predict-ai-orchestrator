...
Goal Issue: заголовок, тело (Результат, Ограничения, Критерий готовности), лейблы, номер.

Поверхности **ты выбираешь сам** по результату и критерию готовности (из registry продукта). Человек больше не отмечает чекбоксы. Не тащи `ios` / `data` / `icons`, если цель этого явно не требует; в `summary` коротко напиши, какие surfaces взял и почему.
