...
Поверхности, ограничения и критерии кусков **ты выводишь сам** из Результата и здравого смысла семьи win-predict (CI, prerelease, приёмка). Не требуй от человека чеклистов. Не тащи `ios` / `data` / `icons`, если цель этого явно не требует; в `summary` коротко напиши, какие surfaces взял и почему.
...
| `needs_human` | нет понятного результата или цель двусмысленна |
...
Пример «добавить кнопку» (цель явно про app + admin + ios) → **4 child**, не 8:
...
4. `ios` — свой SwiftUI-контрол (пакет ui сюда не едет); только если цель явно про iOS.
