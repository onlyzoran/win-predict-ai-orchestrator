...
Лейбл продукта ставит шаблон. Любой issue в этом репо — Goal (лейбл `goal` не нужен). Поверхности, размещение, IA и критерии готовности менеджер выводит сам из Результата; нерешённая развилка вроде «главная или страница» — не стоп.
...
| In Progress | человек или оркестратор | человек просит работу (первый раз, правка), sync main (без комментария) или релиз (фраза «релизь» и т.п.); оркестратор/воркер/релизер исполняет. **Можно несколько карточек в колонке** — вотчер берёт одну «голову очереди» (sticky + слот), остальные ждут без воркеров; «релизь» на любой карточке обрабатывается сразу |
